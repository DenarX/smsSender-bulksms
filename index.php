...
/**
 * Class for batch sending sms by api.bulksms.com
 * @author github.com/DenarX
 */
class Sender
{
    public $r;
    ...
    {
        $this->endpoint = "https://api.bulksms.com/v1/messages/?longMessageMaxParts=160";
        $this->headers = array_merge(['Content-Type:application/json'], (array) ($opt['headers'] ?? null));
    }
    public function send(array $msgList = [])
    {
        $this->setData($msgList);
        return $this->exec();
    }
    public function setData(array $msgList = [])
    {
        $this->data = [];
        foreach ($msgList as $phone => $message) {
            $phone = '+' . preg_replace('/\D/', '', $phone);
            if (preg_match('/^\+\d{10,15}$/', $phone)) { //validate phone number
    ...
    public function exec()
    {
        ...
        $this->r['body'] = curl_exec($this->сh);
        $responseInfo = curl_getinfo($this->сh);
        $this->r['header'] = $responseInfo['request_header'];
        $this->r['status'] = (int) $responseInfo['http_code'];
        $this->r['error'] = curl_error($this->сh);
        curl_close($this->сh);
        return $this->r;
    }
}
